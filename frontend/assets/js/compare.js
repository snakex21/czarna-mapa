...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const buildColumn = (data, columnIndex) => {
    ...
    if (treeBtn) {
      ...
      treeBtn.addEventListener("click", async () => {
        ...
        try {
          const treeData = await invoke('pobierz_genealogie_wlasciciela', { klucz: data.unikalny_klucz });
  ...
  const fetchPromises = ownerKeys.map(async (key) => {
    try {
      const data = await invoke('pobierz_wlasciciela', { klucz: key });
      if (data.error) {
