/* ==========================================================================
   ...
   Opis: Główny skrypt interaktywnej mapy katastralnej gminy Czarna.
         ...
         integracją z API backendowym (Tauri desktop).
   ========================================================================== */

// Tauri API bridge - invoke() is defined globally by api.js
// window.MAP_CONFIG will be loaded asynchronously from the database
...
async function initializeApp() {
    ...
    /* Załaduj konfigurację mapy z Tauri backendu przed inicjalizacją mapy */
    if (!window.MAP_CONFIG) {
        try {
            window.MAP_CONFIG = await invoke('pobierz_konfiguracje_mapy');
        } catch (err) {
            console.warn('⚠️ Nie można załadować konfiguracji mapy, używam domyślnych:', err);
        }
    }
...
/**
 * Pobiera dane z API (Tauri invoke) i buduje interfejs użytkownika.
 * Obsługuje stany ładowania i błędy.
 */
async function fetchDataAndBuildInterface() {
    ...
    /* Równoległe pobieranie danych przez Tauri invoke */
    Promise.all([
        invoke('pobierz_obiekty_widok', { swLat: -90, swLng: -180, neLat: 90, neLng: 180 }),
        invoke('pobierz_wszystkich_wlascicieli'),
    ])
        .then(([dzialkiRaw, wlascicieleRaw]) => {
            ...
            /* Konwersja nowego formatu API na format oczekiwany przez resztę kodu */
            allParcelsData = convertDzialkiToGeoJSON(dzialkiRaw);
            allOwnersData = convertWlascicieleToOwners(wlascicieleRaw);
            buildOwnerParcelLinks(allParcelsData, allOwnersData);
...
/**
 * Konwertuje surowe dane działek z Tauri API na tablicę obiektów GeoJSON.
 * @param {Array} dzialki - Surowe dane z invoke('pobierz_obiekty_widok')
 * @returns {Array} Tablica obiektów GeoJSON Feature
 */
function convertDzialkiToGeoJSON(dzialki) {
    return dzialki.map(d => {
        let geometry = d.geometria_geojson;
        ...
        return {
            ...
            geometry: geometry,
            properties: {
                numer_obiektu: d.nazwa_lub_numer,
                kategoria: d.kategoria,
                wlasciciele: d.wlasciciele || []
            }
        };
    });
}
...
/**
 * Konwertuje surowe dane właścicieli z Tauri API na format oczekiwany przez panel.
 * @param {Array} wlasciciele - Surowe dane z invoke('pobierz_wszystkich_wlascicieli')
 * @returns {Array} Tablica właścicieli w starym formacie
 */
function convertWlascicieleToOwners(wlasciciele) {
    return wlasciciele.map(w => ({
        id: w.id,
...
/**
 * Buduje powiązania właściciel-działka w obu kierunkach.
 * Wypełnia dzialki[].properties.wlasciciele oraz owners[].dzialki_rzeczywiste/protokol
 * na podstawie danych z API lub przez dopasowanie przestrzenne.
 * @param {Array} parcels - Tablica GeoJSON (zmodyfikowana in-place)
 * @param {Array} owners - Tablica właścicieli (zmodyfikowana in-place)
 */
function buildOwnerParcelLinks(parcels, owners) {
    const ownerMap = new Map(owners.map(o => [o.id, o]));
    const ownerByKey = new Map(owners.map(o => [o.unikalny_klucz, o]));

    parcels.forEach(parcel => {
        if (!parcel.properties) return;
        const pOwners = parcel.properties.wlasciciele;
        if (!Array.isArray(pOwners) || pOwners.length === 0) return;

        pOwners.forEach(po => {
            const owner = ownerByKey.get(po.unikalny_klucz) || ownerMap.get(po.id);
            if (!owner) return;

            const parcelRef = { id: parcel.id };
            const typ = po.typ_posiadania;
            if (typ === 'własność rzeczywista') {
                owner.dzialki_rzeczywiste.push(parcelRef);
            } else {
                owner.dzialki_protokol.push(parcelRef);
            }
        });
    });
}
...
function setupOwnerPanel() {
    ...
    const handleOwnerClick = (ownerKey) => {
        if (!isInCompareMode) {
            window.location.href = `views/wlasciciele/protokol.html?ownerId=${ownerKey}`;
        } else {
    ...
    const handleCompareMode = (ownerKey) => {
        ...
        if (selectedForCompare.length === 2) {
            window.location.href = `views/wlasciciele/compare.html?owners=${selectedForCompare.join(",")}`;
        }
    };
...
async function handleShowHouseByOwnerKeyFromURL() {
    ...
    let ownerData = null;
    try {
        ownerData = await invoke('pobierz_wlasciciela', { klucz: ownerKey });
    } catch (e) {
